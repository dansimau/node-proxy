...
exports.createServer = function(options, handler) {
	return http.createServer(function(client_request, response) {
		...
		utils.log(request_time + " - Request: " + client_request.method + " " + client_request.url + ", " + client_request.connection.remoteAddress);
		...
		if (cache_stale) {
			...
			// Do upstream request
			utils.log(request_time + " - Backend: " + target + ":" + target_port + " " + client_request.method + " " + client_request.url);
