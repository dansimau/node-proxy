...
function DiskCache(options) {
	this.options = options || {};
	fs.stat(this.options.path, function(err, stats) {
		if (err)
			throw err;
		if (!stats.isDirectory())
			throw 'Cache directory: not a directory (' + this.options.path + ')';
	});
};
...
DiskCache.prototype.get = function(key) {
	...
	try {
		...
		value = fs.readFileSync(this.options.path + '/' + hash[0] + '/' + hash, 'utf8');
	}
	catch (err) {
		return false;
	}
...
DiskCache.prototype.set = function(key, value) {
	...
	try {
		fs.statSync(this.options.path + '/' + hash[0]);
	}
	catch (err) {
		fs.mkdirSync(this.options.path + '/' + hash[0], 0770);
	}
	fs.writeFile(this.options.path + '/' + hash[0] + '/' + hash, value, function(err) {
		if (err) {
			// Handle write error
		}
	});
